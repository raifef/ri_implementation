...
def build_main() -> None:
    _show(estimate_cost("google-rl-v4-build-plant-ensemble",epochs=0,plants=24))
    _show(build_plant_ensemble())


def splits_main() -> None:
    _show(estimate_cost("google-rl-v4-freeze-synthetic-splits",epochs=0,plants=24))
    _show(freeze_synthetic_splits())


def ppo_main() -> None:
    _show(estimate_cost("google-rl-v4-validate-ppo",epochs=100,plants=1))
    _show(validate_ppo())


def decompose_main() -> None:
    epochs=_epochs("Pre-amendment stability decomposition",96)
    _show(estimate_cost("google-rl-v4-decompose-stability",epochs=epochs,plants=12))
    _show(decompose_stability(epochs=epochs))


def metric_main() -> None:
    _show(estimate_cost("google-rl-v4-validate-stability-metric",epochs=0,plants=0))
    _show(stability_metric_contract())


def amendment_main() -> None:
    epochs=_epochs("One-variable amendment study",96)
    _show(estimate_cost("google-rl-v4-run-amendment-study",epochs=epochs,plants=5,cells=8))
    _show(run_amendment_study(epochs=epochs))


def recovery_main() -> None:
    epochs=_epochs("Recovery by frozen spoil severity",180)
    _show(estimate_cost("google-rl-v4-run-randomized-recovery",epochs=epochs,plants=2,cells=3))
    _show(run_randomized_recovery(epochs=epochs))


def steering_main() -> None:
    epochs=_epochs("Dense steering phase",96)
    _show(estimate_cost("google-rl-v4-run-steering-phase",epochs=epochs,plants=2,cells=21))
    _show(run_steering_phase(epochs=epochs))


def scaling_main() -> None:
    epochs=_epochs("Sparse convergence scaling",28)
    _show(estimate_cost("google-rl-v4-run-convergence-scaling",epochs=epochs,plants=42))
    _show(run_convergence_scaling(epochs=epochs))


def scorecard_main() -> None:
    epochs=_epochs("Development validation scorecard",120)
    _show(estimate_cost("google-rl-v4-run-development-scorecard",epochs=epochs,plants=12))
    _show(run_development_scorecard(epochs=epochs))


def freeze_main() -> None:
    _show(estimate_cost("google-rl-v4-freeze-certification",epochs=0,plants=12))
...
def certification_main() -> None:
    ...
    args=parser.parse_args()
    _show(estimate_cost("google-rl-v4-run-certification",epochs=args.epochs,plants=12,certification=True))
