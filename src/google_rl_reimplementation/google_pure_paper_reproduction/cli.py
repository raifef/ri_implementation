...
from .reporting import audit_all, next_user_commands, reproduction_completeness, reproduction_overview, status
...
def _family_main(family: str, action: str, panel: str | None = None) -> None:
    parser=argparse.ArgumentParser(prog=f"google-rl-paper-{(panel or family.lower()).replace('_','-')}-{action}")
    ...
    elif action=="acquire": result=acquire(protocol,max_shards=args.max_shards,execute_paper_scale=args.execute_paper_scale)
...
def _nonfigure_compare(family: str) -> None:
    ...
    _print({"experiment_family":family,"mode":args.mode,"numeric_checks":__import__("google_rl_reimplementation.google_pure_paper_reproduction.comparison_metrics",fromlist=["family_checks"]).family_checks(family,validation),"verdict":"SMOKE_RENDER_ONLY" if args.mode=="smoke" else "QUALITATIVE_MATCH_ONLY"})
...
def completeness_main() -> None: reproduction_overview();next_user_commands();_print(reproduction_completeness())
