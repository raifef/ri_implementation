...
EXPECTED=("evidence_gate_contract.json","experiment_family_contract.json","natural_drift/raw_traces.npz","natural_drift/psd_results.npz","natural_drift/summary.json","natural_drift/report.md","natural_drift/figure.png",
 ...
 "paper_claim_registry.json","paper_claim_registry.md","paper_comparison/comparison.json","paper_comparison/report.md","supersession_manifest.json","supersession_manifest.md")
...
def validate_manifests()->dict[str,Any]:
    ...
    for path in sorted((base/"validation").glob("*.json")):
      ...
      rows.append({"path":path.relative_to(repository_root()).as_posix(),"old_status":value.get("status"),"old_final_evidence":generic,"v8_status":"INVALID_DIAGNOSTIC" if mode in {"smoke","validation"} else ("CLAIM_NOT_SUPPORTED" if not value.get("valid") else "PAPER_ANCHORED_SYNTHETIC_EVIDENCE"),"prompt1_hash_bound":prompt1["artifact_hash"]})
    superseded=[{"legacy_artifact":"claim_registry.json","superseded_by":"paper_claim_registry.json","reason":"old registry lacked required claim statuses and enforced anti-conflation"},      {"legacy_artifact":"step_response/summary.json","superseded_by":"step_response/results.json","reason":"old response normalized crossings by achieved motion"},
