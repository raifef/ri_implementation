...
def natural_main(): _run(run_natural_drift,"google-rl-evidence-v8-run-natural-drift")
def step_main(): _run(run_step_response,"google-rl-evidence-v8-run-step-response")
def recovery_main(): _run(run_recovery,"google-rl-evidence-v8-run-recovery")
def figure5b_main(): _run(run_figure5b,"google-rl-evidence-v8-run-figure5b")
def figure5c_main(): _run(run_figure5c,"google-rl-evidence-v8-run-figure5c")
def claims_main(): _show(build_claim_registry())
def comparison_main(): _show(build_paper_comparison())
def status_main():
 preflight=build_protocol_preflight();files=sorted(str(p.relative_to(root())) for p in root().rglob("*.json")); print(json.dumps({"artifact_root":"artifacts/google_pure_evidence_v8","json_artifacts":files,"superseded_legacy_artifacts":["claim_registry.json","step_response/summary.json","recovery/summary.json"],"protocol_gate_pass":preflight["protocol_gate_pass"],"reference_acquisition_permitted":preflight["reference_acquisition_permitted"],"missing_files":preflight["missing_files"]},indent=2))
