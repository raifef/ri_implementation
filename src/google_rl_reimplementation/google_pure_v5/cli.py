...
def source_main() -> None:
    _run("google-rl-v5-audit-source-compliance", source_compliance_map)


def baseline_main() -> None:
    _run("google-rl-v5-audit-baseline", audit_baseline)


def algorithm_main() -> None:
    _run("google-rl-v5-validate-algorithm", validate_algorithm)


def static_main() -> None:
    epochs = _epochs("Static optimization and no-drift gates", 180, 80)
    _run("google-rl-v5-run-static-tests", run_static_tests, epochs=epochs, experiments=3, kwargs={"epochs": epochs})


def injected_main() -> None:
    epochs = _epochs("Injected-drift stability", 360, 120)
    _run("google-rl-v5-run-injected-drift", run_injected_drift, epochs=epochs, experiments=6, kwargs={"epochs": epochs})


def natural_main() -> None:
    epochs = _epochs("Natural-drift spectral suppression", 768, 256)
    _run("google-rl-v5-run-natural-drift-spectral", run_natural_drift_spectral, epochs=epochs, experiments=7, kwargs={"epochs": epochs})


def separation_main() -> None:
    _run("google-rl-v5-audit-test-separation", audit_test_separation)


def step_main() -> None:
    _run("google-rl-v5-run-step-response", run_step_response)


def steering_main() -> None:
    epochs = _epochs("Fixed-entropy steering phase diagram", 300, 180)
    _run("google-rl-v5-run-steering-phase", run_steering_phase, epochs=epochs, experiments=30, kwargs={"epochs": epochs})


def recovery_main() -> None:
    epochs = _epochs("Frozen-severity randomized-policy recovery", 1000, 200)
    _run("google-rl-v5-run-randomized-recovery", run_randomized_recovery, epochs=epochs, experiments=3, kwargs={"epochs": epochs})


def scaling_main() -> None:
    epochs = _epochs("Actual sparse convergence scaling", 16, 8)
    _run("google-rl-v5-run-convergence-scaling", run_convergence_scaling, epochs=epochs, experiments=14, scaling=True, kwargs={"epochs": epochs})


def scorecard_main() -> None:
    _run("google-rl-v5-run-development-scorecard", run_development_scorecard)


def freeze_main() -> None:
    _run("google-rl-v5-freeze-certification", freeze_certification)
...
def certification_main() -> None:
    ...
    args = parser.parse_args()
    _run("google-rl-v5-run-certification", run_certification, epochs=args.epochs, experiments=12, certification=True,
