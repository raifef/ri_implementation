...
def prior_evidence_manifest() -> dict[str, Any]:
    root = repository_root()
    trees = {
        "v2_source": root / "src/google_rl_reimplementation/google_reproduction",
        "v3_source": root / "src/google_rl_reimplementation/google_reproduction_v3",
        "v4_source": root / "src/google_rl_reimplementation/google_synthetic_v4",
...
def dependency_audit() -> dict[str, Any]:
    ...
    files = sorted((repository_root() / "src/google_rl_reimplementation/google_pure_v5").glob("*.py"))
    ...
    for path in files:
        ...
        for name in found:
            ...
            lower = name.lower()
            if any(token in lower for token in ("stage", "forecast", "mpc", "outside_workflow", "residual", "google_synthetic_v4")):
...
def audit_test_separation() -> dict[str, Any]:
    ...
    injected_source = (repository_root() / "src/google_rl_reimplementation/google_pure_v5/injected_drift_test.py").read_text(encoding="utf-8")
    natural_source = (repository_root() / "src/google_rl_reimplementation/google_pure_v5/natural_drift_spectral_test.py").read_text(encoding="utf-8")
    ...
    payload = {
        ...
        "generator_locations": {
            "injected": "src/google_rl_reimplementation/google_pure_v5/injected_drift_test.py:generate_injected_tape",
            "natural": "src/google_rl_reimplementation/google_pure_v5/natural_drift_spectral_test.py:generate_natural_drift",
        },
