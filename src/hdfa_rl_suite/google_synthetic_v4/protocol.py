...
def _immutability_snapshot() -> dict[str, Any]:
    ...
    paths: list[Path] = []
    for relative in ("src/hdfa_rl_suite/google_reproduction","src/hdfa_rl_suite/google_reproduction_v3",
...
def freeze_certification() -> dict[str, Any]:
    ...
    if not all_gates:
        ...
        return payload
    root = repository_root()
    protected = list((root/"src/hdfa_rl_suite/google_synthetic_v4").glob("*.py")) + list(config_dir().glob("*.yaml"))
    ...
    payload = {
        ...
        "blocked_until_certified":["reduced-budget equivalence","staged HDFA comparison"],
