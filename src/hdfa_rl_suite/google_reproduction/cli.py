...
def finetuning_main() -> None:
    _run_development("hdfa-google-v2-run-finetuning-development", "fine_tuning", run_finetuning, default_seed=7901, default_epochs=240, include_sensitivity=True)


def drift_main() -> None:
    _run_development("hdfa-google-v2-run-drift-stability-development", "drift_stability", run_drift_stability, default_seed=7901, default_epochs=600)


def step_main() -> None:
    _run_development("hdfa-google-v2-run-step-response-development", "step_response", run_step_response, default_seed=7902, default_epochs=520)


def recovery_main() -> None:
    _run_development("hdfa-google-v2-run-randomized-recovery-development", "randomized_recovery", run_randomized_recovery, default_seed=7903, default_epochs=1400)


def steering_main() -> None:
    _run_development("hdfa-google-v2-run-steering-phase-development", "steering_phase", run_steering_phase, default_seed=7901, default_epochs=360)


def scaling_main() -> None:
    _run_development("hdfa-google-v2-run-scaling-development", "scaling", run_scaling, default_seed=7902, default_epochs=None)
...
def certification_main() -> None:
    parser = argparse.ArgumentParser(prog="hdfa-google-v2-run-certification")
...
def staged_recheck_main() -> None:
    final_path = artifact_directory() / "final_certification.json"
    final = json.loads(final_path.read_text(encoding="utf-8")) if final_path.exists() else {}
    if final.get("outcome") != "GOOGLE_STYLE_OPEN_REPRODUCTION_CERTIFIED":
        raise SystemExit("small staged recheck is blocked until the paper-scale reference is certified")
    raise SystemExit("certified reference detected; create a separate small development preregistration before invoking staged code")
