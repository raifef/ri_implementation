...
from .direct_path import protocol_identity_reasons
...
def validate_protocol(protocol: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if protocol["mode"] in {"reference", "paper-scale"}:
        reasons.extend(protocol_identity_reasons(protocol))
        for field in ("controller_mode", "controller_hash", "controller_code_hash", "plant_hash", "graph_hash"):
            if provenance.get(field) != protocol.get(field):
                reasons.append(f"merged provenance does not inherit protocol {field}")
    ...
    paper_comparable = bool(metrics.get("paper_comparable", False))
    final = final_evidence_allowed(mode=protocol["mode"], complete=merged["complete"],
                                   scientifically_valid=valid) and paper_comparable
    ...
    elif metrics.get("outcome") in {"RECOVERY_NOT_REACHED_WITHIN_HORIZON","STEP_TARGET_NOT_REACHED_WITHIN_HORIZON"}: status = metrics["outcome"]
    elif valid and not paper_comparable: status = "SOURCE_STRUCTURE_VALIDATED_NOT_NUMERICALLY_PAPER_COMPARABLE"
    ...
    result = {"schema_version": "google-paper-validation.v1", "experiment_family": protocol["experiment_family"],
              ...
              "controller_mode":protocol["controller_mode"],"controller_hash":protocol["controller_hash"],
              "controller_code_hash":protocol["controller_code_hash"],"parameterization":protocol["parameterization"],
              "plant_hash":protocol["plant_hash"],"graph_hash":protocol["graph_hash"],
              ...
              "scientifically_valid": valid, "paper_comparable":paper_comparable,"staged_controller_run": False,
              "provenance_inherited_without_promotion": not final if reasons else True,
              "diagnostic_label": metrics.get("panel_label")}
